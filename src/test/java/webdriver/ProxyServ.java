...
public class ProxyServ extends BaseEntity{
	...
	public static void assertAnalyzedTrafficResponseCode(){
        ...
        try {
			List<HarEntry> arrayRequests = har.getLog().getEntries();
			for (HarEntry entry: arrayRequests) {
				if (entry.getResponse().getStatus()>_400){
...
//        List<HarEntry> arrayRequests = har.getLog().getEntries();
//        for (HarEntry entry: arrayRequests) { 
//        	if (entry.getTime()>timeLimitInMiliSeconds){
//        		logger.warn(String.format("Load time: %1$s", entry.getTime()));
//        		logger.warn("Details: " + entry.getRequest().getMethod() + " " + entry.getRequest().getUrl());
//        	}
//        }
	}
