...
public abstract class Table extends BaseElement implements ITable {
	...
	public int getRowsCount() {
		int countRows = 0;
		try {
			countRows = browser.getDriver().findElementsByXPath(String.format(tbodyTemplate, currentTableId) + "//tr").size();
		} catch (Exception e) {
	...
	public String getCellText(int rowNumber, IColumnsTags columnTag) {
		String cellText = browser.getDriver().findElementByXPath(String.format(cellTemplate, 
	...
	public String getCellText(int rowNumber, IColumnsTags columnTag, int tbodyNum) {
		//Label lbl = new Label(By.xpath(String.format(rowTbodyNumTemplate + "//td[%4$s]/div", currentTableId, tbodyNum, rowNumber, columnTag.toString())), "");
		String cellText = browser.getDriver().findElementByXPath(String.format(rowTbodyNumTemplate + "//td[%4$s]/div", 
	...
	public void clickCell(int rowNumber, IColumnsTags columnTag) {
		browser.getDriver().findElementByXPath(
	...
	public void assertRecordExist(IColumnsTags tag, String cellValue) {
		...
		AssertJUnit.assertTrue(
				String.format("Record with cell text '%1$s' in column '%2$s' is not present", cellValue, tag),
				(row != 0));
	...
	public int getRowNumberWithoutValue(IColumnsTags primaryTag, String value) {
		...
		for (int i = 1; i <= countRows; i++) {
			...
			//String primLocator = String.format(rowColumnTemplate, i, primaryTag.toString(), value);
